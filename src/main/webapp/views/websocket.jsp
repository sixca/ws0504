...
<script>
    let websocket = {
        ...
        connect:function(){
            var sid = this.id;
            var socket = new SockJS('http://127.0.0.1:8088/ws');  // 웹소켓 접속정보
            this.stompClient = Stomp.over(socket);  // 이 소켓을 넣어주고

            this.stompClient.connect({}, function(frame) {   //접속 시도
                websocket.setConnected(true);
                console.log('Connected: ' + frame);
                this.subscribe('/send', function(msg) {    //접속이 되고나면, subscribe(받을 준비)를 하게 됨. #all, #me, #to
                    $("#all").prepend(
        ...
        sendAll:function(){
            var msg = JSON.stringify({
                'sendid' : this.id,   //관리자의 id겠지
                'content1' : $("#alltext").val()
            });  //메세지 객체를 만들어서
            this.stompClient.send("/receiveall", {}, msg);   // receiveall에게 보내줌
        },
        sendTo:function(){
            var msg = JSON.stringify({
                'sendid' : this.id,
